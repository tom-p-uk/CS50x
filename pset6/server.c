...
char* indexes(const char* path)
{
    ...
    // check accessibiliy of php file and return phpIndex if it exists
    if(access(phpIndex, F_OK) == 0) 
    {
        return phpIndex;
    }
    
    // check accessibiliy of html file and return htmlIndex if it exists
...
const char* lookup(const char* path)
{
    ...
    // create new string starting from the file extention in 'path' string
    int c = '.';
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    if (strchr(requestTarget, '?') == NULL)
    {
        ...
        query_temp[0] = '\0';
        strncpy(query, query_temp, (strlen(query_temp) + 1));
    ...
    else
    {
        ...
        // set 'query_temp2' to begin after the question mark
        query_temp2 = &query_temp[1];
        strncpy(query, query_temp2, (strlen(query_temp2) + 1));
        
        // copy 'requestTarget' to a temp string and swap out the question mark for terminating null char
        strncpy(abs_path_temp, requestTarget, (strlen(requestTarget) + 1));
